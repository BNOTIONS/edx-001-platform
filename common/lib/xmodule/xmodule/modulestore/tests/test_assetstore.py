...
class TestMongoAssetMetadataStorage(unittest.TestCase):
    ...
    def setup_assets(self, course1_key, course2_key, store=None):
        ...
        if store is not None:
            store.save_asset_metadata(asset1_md, ModuleStoreEnum.UserID.test)
            store.save_asset_metadata(asset2_md, ModuleStoreEnum.UserID.test)
            store.save_asset_metadata(asset3_md, ModuleStoreEnum.UserID.test)
            store.save_asset_metadata(asset4_md, ModuleStoreEnum.UserID.test)
